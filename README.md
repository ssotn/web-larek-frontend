...
- AppState - класс для управления состояниями приложения.
  ...
  Методы:
    ...
    - ```validateOrderForm``` - приватный. валидация полей формы заказа;
    - ```validateContactsForm``` - приватный. валидация полей формы котактов;
     - ```refreshOrder``` - приватный. устанавливает поля заказа по умолчанию.
